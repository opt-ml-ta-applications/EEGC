...
def main():
    ...
    if len(sys.argv)==9:
        ...
        if METRIC=='nonlinear': # 
            ...
            MARGIN=0.015
    start=time.time()
    best_acc,best_epoch=train(dev_index,word_vec,ans,train_data,dev_data,test_data,
    float(L2_penalty),float(MARGIN),
    float(LR),int(T),int(BATCH_SIZE),int(EPOCHES),int(PATIENTS),int(HIDDEN_DIM),METRIC)
    end=time.time()
    print ("Run time: %f s" % (end-start))
    with open('best_result.txt','a') as f:
        f.write('Best Acc: %f, Epoch %d , L2_penalty=%s ,MARGIN=%s ,LR=%s ,T=%s ,BATCH_SIZE=%s ,EPOCHES=%s, PATIENTS=%s, HIDDEN_DIM=%s, METRIC=%s\n' % (best_acc,best_epoch,L2_penalty,
        MARGIN,LR,T,BATCH_SIZE,EPOCHES,PATIENTS,HIDDEN_DIM,METRIC))
    f.close()
